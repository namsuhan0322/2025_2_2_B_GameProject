...
public class CityGenerator : MonoBehaviour
{
    ...
    private void CreateRoadTile(int x, int z)
    {
        GameObject ground = GameObject.CreatePrimitive(PrimitiveType.Cube);
    ...
    private void CreateBuildings()
    {
        for(int x = 0;x < cityWidth; x++)
        {
            for (int z = 0; z < cityHeight; z++)
            {
                // 추후 랜덤으로 타워, 광장, 공원을 추가
                if (!IsRoad(x, z))
                {
                    float rand = Random.value;

                    if (rand < towerChance) CreateTower(x, z);
                    else if (rand < towerChance + plazaChance) CreatePlaza(x, z);
                    else if (rand < towerChance + plazaChance + parkChance) CreatePark(x, z);
    ...
    private void CreateTower(int x, int z)
    {
    ...
    private void CreatePark(int x, int z)
    {
    ...
    private void CreatePlaza(int x, int z)
    {
    ...
    // 건물 색 설정
    private Color GetBuildingColor(int x, int z)
    {
        float disFromCenter = Vector2.Distance(new Vector2(x, z), new Vector2(cityWidth / 2f, cityHeight / 2f));

        if (disFromCenter < cityWidth * 0.2f) return new Color(0.6f, 0.6f, 0.7f);
        else if (disFromCenter < cityWidth * 0.4f) return new Color(0.8f, 0.7f, 0.7f);
        else return new Color(0.7f, 0.6f, 0.5f);
    }
